...
class RESTClient(MyAirClient):
    ...
    async def _gql_query(
        ...
        if not self._country_code and self._id_token:
            ...
            try:
                ...
                _LOGGER.error(
                    "Error decoding id_token into jwt_data. %s: %s", e.__class__.__qualname__, e
                )
    ...
    async def get_sleep_records(self, initial: bool | None = False) -> list[SleepRecord]:
        ...
        try:
            ...
            _LOGGER.error(
                "Error getting Patient Sleep Records. %s: %s", e.__class__.__qualname__, e
            )
    ...
    async def get_user_device_data(self, initial: bool | None = False) -> MyAirDevice:
        ...
        try:
            ...
            _LOGGER.error("Error getting User Device Data. %s: %s", e.__class__.__qualname__, e)
